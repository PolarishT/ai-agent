 ...
 * <ul>
 *   <li>enqueue 首次插入 → 二次复用同一行（不产生重复 NEW 行）</li>
...
class CatalogAttributeOutboxRepositoryTests {
    ...
    @Test
    void enqueueInsertsNewRowOnFirstCall() {
        outboxRepository.enqueue(101L, "EXTRACT_ATTRIBUTES", "{\"triggeredBy\":\"import\"}");

        Optional<CatalogAttributeOutboxRecord> latest = outboxRepository.findLatestByProductId(101L);
        assertThat(latest).isPresent();
        assertThat(latest.get().status()).isEqualTo("NEW");
        assertThat(latest.get().attemptCount()).isZero();
        assertThat(latest.get().eventType()).isEqualTo("EXTRACT_ATTRIBUTES");
    }

    @Test
    void enqueueReusesExistingRowAndResetsAttemptCount() {
        outboxRepository.enqueue(102L, "EXTRACT_ATTRIBUTES", "{}");
        Long firstId = outboxRepository.findLatestByProductId(102L).orElseThrow().id();
        ...
        // 二次入队应复用同一行，状态回到 NEW、attempt_count 清零
        outboxRepository.enqueue(102L, "EXTRACT_ATTRIBUTES", "{\"triggeredBy\":\"manual-retry\"}");

        Optional<CatalogAttributeOutboxRecord> reused = outboxRepository.findLatestByProductId(102L);
        assertThat(reused).isPresent();
        assertThat(reused.get().id()).isEqualTo(firstId);
        assertThat(reused.get().status()).isEqualTo("NEW");
        assertThat(reused.get().attemptCount()).isZero();
        assertThat(reused.get().lastError()).isNull();
        assertThat(jdbcTemplate.queryForObject(
                "SELECT count(*) FROM catalog_attribute_outbox WHERE product_id=?", Integer.class, 102L))
                .as("同一 Product 不应产生重复 outbox 行")
                .isEqualTo(1);
    }

    @Test
    void markSendingIsAtomicAndIdempotent() {
        outboxRepository.enqueue(103L, "EXTRACT_ATTRIBUTES", "{}");
        Long id = outboxRepository.findLatestByProductId(103L).orElseThrow().id();
    ...
    void markSentCompletesTerminalState() {
        outboxRepository.enqueue(104L, "EXTRACT_ATTRIBUTES", "{}");
        Long id = outboxRepository.findLatestByProductId(104L).orElseThrow().id();
        outboxRepository.markSending(id);

        outboxRepository.markSent(id, "MSG-1");

        CatalogAttributeOutboxRecord r = outboxRepository.findLatestByProductId(104L).orElseThrow();
    ...
    void findDispatchableReturnsPendingOrFailedWithDuePeriod() {
        ...
        outboxRepository.enqueue(201L, "EXTRACT_ATTRIBUTES", "{}");       // NEW
        outboxRepository.enqueue(202L, "EXTRACT_ATTRIBUTES", "{}");
        Long sendingId = outboxRepository.findLatestByProductId(202L).orElseThrow().id();
        ...
        outboxRepository.enqueue(203L, "EXTRACT_ATTRIBUTES", "{}");
        Long futureId = outboxRepository.findLatestByProductId(203L).orElseThrow().id();
        outboxRepository.markSending(futureId);
        outboxRepository.markFailed(futureId, "retry-later", referenceNow.plusMinutes(5));

        outboxRepository.enqueue(204L, "EXTRACT_ATTRIBUTES", "{}");
        Long pastFailId = outboxRepository.findLatestByProductId(204L).orElseThrow().id();
        ...
        assertThat(dispatchable)
                .extracting(CatalogAttributeOutboxRecord::productId)
    ...
    void findStuckSendingBeforeReturnsStaleSendingRows() {
        ...
        outboxRepository.enqueue(301L, "EXTRACT_ATTRIBUTES", "{}");
        Long stuckId = outboxRepository.findLatestByProductId(301L).orElseThrow().id();
        ...
        assertThat(outboxRepository.findLatestByProductId(301L).orElseThrow().status()).isEqualTo("FAILED");
    }

    @Test
    void resetForRetryOnlyMovesSendingRows() {
        outboxRepository.enqueue(401L, "EXTRACT_ATTRIBUTES", "{}");
        Long pendingId = outboxRepository.findLatestByProductId(401L).orElseThrow().id();

        outboxRepository.resetForRetry(pendingId, "noop", OffsetDateTime.now(ZoneOffset.UTC));

        assertThat(outboxRepository.findLatestByProductId(401L).orElseThrow().status())
                .as("NEW 行不应被 resetForRetry 修改")
                .isEqualTo("NEW");
    }
}
